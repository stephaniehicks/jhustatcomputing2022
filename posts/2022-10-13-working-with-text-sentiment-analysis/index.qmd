...
Another way of putting it is that it is a set of packages 
...
In this way, you can focus on the problem you are
...
Using a given set of token, we can 
...
tokenize text a data frame is the `unnest_tokens(tbl, output, input)`
function.
...
In addition to the data frame (`tbl`), the function needs two
basic arguments: 

1. `output` or the output column name that will be 
created (e.g. string) as the text is unnested into it
...
He goes to say that back then no one was 
...
quantity to keep track of paragraphs in the original format. 
...
The `nrc` lexicon **categorizes words into multiple categories** of 
...
so this is not an important effect."_
...
analysis. A text the size of many paragraphs can often have positive 
and negative sentiment averaged out to about zero, while sentence-sized 
...
sentiment analysis, we can join the words together using a 
join function. 
...
track of where we are in the narrative, which uses integer division, 
...
In this section, we want to convert our tidy text data constructed with the 
`unnest_tokens()` function (useable by packages in the tidyverse) into a 
different format that can be used by packages for natural language processing 
or other types of machine learning algorithms in non-tidy formats. 

In the figure below, we see how an analysis might switch between tidy and non-tidy data structures and tools. 

```{r, echo=FALSE, out.width = '90%', fig.cap=" A flowchart of a typical text analysis that combines tidytext with other tools and data formats, particularly the tm or quanteda packages. This chapter shows how to convert back and forth between document-term matrices and tidy data frames, as well as converting from a Corpus object to a text data frame."}
...
To introduce some of these toosl, we first need to introduce **document-term matrices**, as well as **casting** a tidy data frame into a sparse matrix. 


### Document-term matrix

One of the most common structures that text mining packages work with 
is the **document-term matrix** (or DTM). This is a matrix where:
...
With these matricies, you can start to leverage the NLP methods and software. 
For example, in text mining, we often have collections of documents, such as blog posts or news articles, that we’d like to divide into natural groups so that we can understand them separately. 
